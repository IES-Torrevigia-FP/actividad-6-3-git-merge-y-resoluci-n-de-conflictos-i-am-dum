...
Extracto Reflexion 6.3 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.3) se califica con rubrica: Merge y conflictos (max 4), Evidencias (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece branch_evidence.detected_only_in_evidence con elementos, NO afirmes que esas ramas faltan.
  Formula el hallazgo como: "no detectadas como refs remotas, pero si evidenciadas en archivos".
- Si merge_checks.conflict_checkpoint.markers_detected es true y merge_checks.final_historia.has_conflict_markers es false,
  reconoce explicitamente que hubo conflicto documentado y resolucion final limpia.
- Si merge_checks.merge_commit_detected es false pero hay evidencias en checkpoints, evita afirmar categoricamente que no hubo merge;
  habla de limitacion de deteccion automatica y prioriza la evidencia documental.
- Si branch_evidence.missing_expected_features esta vacio, no menciones faltas de ramas.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-3.md'))
